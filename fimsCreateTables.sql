...
BEGIN
    ...
    EXECUTE
                                                    'INSERT INTO ' || audit_table_name || ' (user_name, ts, action, config, ' ||
                                                    audit_table_foreign_key_column || ') ' ||
                                                    'VALUES (' || quote_literal(audit_session_user) ||
                                                    ', CURRENT_TIMESTAMP, ' || quote_literal(substring(TG_OP, 1, 1)) ||
...
CREATE TRIGGER network_config_history
    ...
    ON networks
    FOR EACH ROW
EXECUTE PROCEDURE config_history('network_config_history', 'network_id');
...
CREATE TRIGGER project_config_history
    ...
    ON project_configurations
    FOR EACH ROW
EXECUTE PROCEDURE config_history('project_config_history', 'project_configuration_id');
...
CREATE TRIGGER tsvector_update
    ...
    ON expeditions
    FOR EACH ROW
EXECUTE PROCEDURE expeditions_tsv_trigger();
...
CREATE TRIGGER set_users_last_login
    ...
    ON oauth_tokens
    FOR EACH ROW
EXECUTE PROCEDURE set_users_last_login();
...
DROP TABLE IF EXISTS sra_submissions;

CREATE TABLE sra_submissions
(
    id             SERIAL    NOT NULL PRIMARY KEY,
    project_id     INTEGER   NOT NULL REFERENCES projects (id) ON DELETE CASCADE,
    expedition_id  INTEGER   NOT NULL REFERENCES expeditions (id) ON DELETE CASCADE,
    user_id        INTEGER   NOT NULL REFERENCES users (id) ON DELETE CASCADE,
    submission_dir TEXT      NOT NULL,
    message        TEXT,
...
CREATE TRIGGER update_sra_submissions_modtime
    BEFORE INSERT OR UPDATE
    ON sra_submissions
    FOR EACH ROW
EXECUTE PROCEDURE update_modified_column();
CREATE TRIGGER set_sra_submissions_createdtime
    BEFORE INSERT
    ON sra_submissions
    FOR EACH ROW
EXECUTE PROCEDURE set_created_column();
