    ...
    public void saveDataset(Dataset dataset, int networkId) {
        try {
            for (RecordSet recordSet : dataset) {
                Timer.getInstance().lap("saving recordset: " + recordSet.conceptAlias());
                ...
                if (!insertParams.isEmpty()) {
                    if (executeReturning) {
                        ...
                        ListUtils.partition(insertParams, 4000).forEach(chunk -> {
                            jdbcTemplate.batchUpdate(
                                    StringSubstitutor.replace(sqlString, tableMap),
                                    chunk.toArray(new HashMap[chunk.size()])
                            );
                        });
                    }
                }
                ...
                Timer.getInstance().lap("finished saving recordset: " + recordSet.conceptAlias());
            }
        } catch (JsonProcessingException e) {
            throw new FimsRuntimeException(UploadCode.DATA_SERIALIZATION_ERROR, 500);
        }
