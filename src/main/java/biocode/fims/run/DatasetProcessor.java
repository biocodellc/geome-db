...
public class DatasetProcessor {
    ...
    public boolean validate(boolean upload) {
        ...
        Timer.getInstance().lap("preparing to build DatasetBuilder");

        dataset = datasetBuilder.build();
        Timer.getInstance().lap("DatasetBuilder built");
    ...
    public boolean upload() {
        ...
        Timer.getInstance().lap("running dataset actions");
        for (DatasetAction a : actions) {
            a.onSave(project, dataset);
        }
        Timer.getInstance().lap("finished running dataset actions");

        recordRepository.saveDataset(dataset, project.getNetwork().getId());
        Timer.getInstance().lap("finished saving dataset");
